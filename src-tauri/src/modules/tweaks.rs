...
//! Reads from /proc/sys and /sys, applies via pkexec sysctl
...
/// Get all tweaks organized by category
#[tauri::command]
pub fn get_tweaks() -> Result<Vec<TweakCategory>> {
...
#[tauri::command]
pub fn apply_tweak(tweak_id: String, value: String) -> Result<String> {
    match tweak_id.as_str() {
        ...
        | "tcp_congestion" | "tcp_fastopen" | "tcp_mtu_probing" => {
            ...
            if tweak_id == "tcp_congestion" && value == "bbr" {
                let _ = privileged::run_privileged_shell("modprobe tcp_bbr");
            }

            privileged::run_privileged("sysctl", &["-w", &format!("{}={}", key, value)])?;
...
#[tauri::command]
pub fn apply_all_recommended() -> Result<Vec<String>> {
    ...
    for (key, value) in memory_tweaks {
        if let Ok(_) = privileged::run_privileged("sysctl", &["-w", &format!("{}={}", key, value)]) {
